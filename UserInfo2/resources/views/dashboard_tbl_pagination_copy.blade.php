...
<table id="custom_datatable" class="table table-bordered table-hover table-responsive" width="100%">
    ...
    <tbody>
        @if (!empty($searchresults) || sizeOf($searchresults) > 0 || $searchresults != '' || $searchresults != null)
            ...
            @foreach ($searchresults as $key => $data)
                @php
                    ...
                    if (
                        str_contains($searchresults[$lastKey]->computerName, 'RUC') ||
                        str_contains($searchresults[$lastKey]->computerName, 'NUC') ||
                        str_contains($searchresults[$lastKey]->computerName, 'TUC')
                    ) {
                        // $installDate = $data->installDate;


                        $installDate = $installDate_tmp;
                ...
                @endphp
                <tr>
                    <td class="text-center"> <strong title="Sr.No"> {{ $rowcnt }}</strong></td>
                    ...
                    <td class="col-3" data-order="{{ $data->customerNumber }}">
                        ...
                        <strong>Name: </strong><a href="javascript:void(0)"
                            onclick="OpenUserDetails({{ $newCustNo }})" data-toggle="tooltip" data-placement="bottom"
                        ...
                        <strong>
                            Email: <a href="javascript:void(0)" onclick="OpenUserDetails({{ $newCustNo }})"
                                data-toggle="tooltip" data-placement="bottom" title="User Info"><span
                                    id="cstEmail{{ $newCustNo }}">{{ !empty($data->emailID) ? $data->emailID : 'Add EmailID' }}</span></a>
                        </strong>
                        <br />
                            ...
                            <div class="col-7">
                                ...
                                @if ($SpecialKeyMsg)
                                    <strong class='text-danger'
                                        title="{{ $SpecialKeyMsg }} | {{ $addedBy }} | {{ $indate }}">[{{ $SpecialKeyMsg }}]</strong>
                                @endif
                                @if ($packDays)
                                    <strong class='text-danger'
                                        title="{{ $packDays }}">[{{ $packDays }}]</strong>
                                @endif
                                {!! $licExpired_div !!}
                                @if ($log_div)
                                    {!! $log_div !!}
                                ...
                                {!! $pio_div !!}
                                <br />
                                <strong>IC:</strong> <span>{{ $Install_Code }} <i class="fa fa-clipboard"></i> </span>
                                &nbsp;
                                <span class="badge badge-sm badge-success"
                                    title="IC based on {{ $IC_basedOn }}">{{ $IC_basedOn }}</span>
                                <br />
                                <strong>UC:</strong>
                                <span @if ($isLastUCCodeMatched == true) class="highUc" @endif>{!! $unlockCode_div !!}
                                </span> | @if (in_array($unlockCode, $unlockcode_arr))
                                    <strong title="UC repeated {{ $count_uc[$unlockCode] }} times. ">
                                        {{ $count_uc[$unlockCode] }}</strong>
                                @endif

                                @if ($rowcnt_index > 0 && $search_by == 'lic')
                                    | <button class="btn-sm btn btn-outline-primary addSureBlockChkbx"
                                        data-serialno="{{ $SerialNo }}" data-custno="{{ $newCustNo }}"
                                ...
                                <span class="rem-days" style="{{ $style }}"> <strong>[{{ $DaysRemain }}]
                                    </strong>
                                </span><br>
                                ------------------------------------- <br>

                                Activation:{{ $AuthoriedByName }} | Install By: {{ $installBy }}<br />
                                ...
                                @if ($fInDate)
                                    <br />
                                    @if ($search_by == 'lic')
                                        @if ($searchresult_key == $required_index)
                                    ...
                                    @else
                                        <strong>InDate: </strong>
                                    @endif
                                    <span class='indate text-danger'><strong>{{ $fInDate }}</strong> </span>
                                @endif
                                @if ($SUCDiv)
                                    {!! $SUCDiv !!}
                                @endif
                            ...
                            <div class="col-5">
                                ...
                                @if ($newContent)
                                    <div class="col-12"
                                        ...
                                        @endphp
                                        @foreach ($explode_text as $key1 => $value1)
                                            {{ $value1 }} <br>
                                        @endforeach
                    ...
                    <td class="col-4">
                        ...
                        @if (
                            $LoggedUser == 'sumeet' ||
                                ...
                                $LoggedUser == 'vikramkumar' ||
                                $LoggedUser == 'ganeshw')
                            <button class="btn btn-sm btn-primary fetchKey my-3" data-serialno={{ $SerialNo }}
        ...
        @if (
            ...
            @foreach ($searchresults_inactive as $key_inactive => $data)
                @php
                ...
                <tr style="vertical-align: bottom;">
                    <td class="text-center"> <strong title="Sr.No"> {{ $key_inactive + 1 }}</strong></td>
                    </td>
                    <td>
        ...
        <div class="col-md-6">
            ...
            <div class="divHwDetails" id="Notes">
                @if (!empty($objrs))
                    ...
                    <span onmouseover='matchWithReactRequest(this);'>{{ $Install_Code }}</span><br />
                    ...
                    <span onmouseover="matchWithReactRequest(this);">{{ !empty($objrs) ? $objrs->Model : '' }}</span>
                    {{-- @else --}}
                @endif
